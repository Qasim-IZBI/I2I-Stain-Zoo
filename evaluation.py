...
Metrics:
  - fid: Distribution-level Fréchet distance (unpaired, InceptionV3 pool3 2048-d features)
...
Example (FID):
  python evaluation.py --metric fid --path_real data/CD13 --path_fake results/he_to_cd13 --device cuda
...
Notes:
- patch_ssim requires paired images matched by filename.
...
def list_paired_images(
    ...
    """Match images by relative path between two folders, optionally filtered by tissue stems.

    Relative-path matching preserves WSI folder identity (e.g. 001/images/0000001.tif)
    so tiles with the same filename in different WSI subdirectories are correctly paired
    rather than collapsed to one tile per unique basename.  For flat directories the
    relative path equals the basename, so behaviour is unchanged.

    Falls back to basename matching if no common relative paths are found (e.g. when
    one directory is structured and the other is flat).
    """
    real_relmap = {os.path.splitext(os.path.relpath(p, path_real))[0]: p
                   for p in list_images(path_real)}
    fake_relmap = {os.path.splitext(os.path.relpath(p, path_fake))[0]: p
                   for p in list_images(path_fake)}

    common = sorted(set(real_relmap) & set(fake_relmap))

    if not common:
        # Fallback: match by bare stem (handles flat-vs-structured dir mismatches)
        real_map = {os.path.splitext(os.path.basename(p))[0]: p for p in real_relmap.values()}
        fake_map = {os.path.splitext(os.path.basename(p))[0]: p for p in fake_relmap.values()}
        ...
        pairs = [(real_relmap[k], fake_relmap[k]) for k in common]
...
def compute_regen_error_precomputed(
    path_A: str,
    path_A_regen: str,
    image_size: int = 256,
    ...
    """
    Compute regen MAE from pre-computed A' tiles: error = |A − A'|.

    Works for any model (including cyclediffusion) as long as A' tiles have
    already been generated by infer_B2A_all.sh (or equivalent B→A inference).
    Tiles are matched by filename stem; unmatched files are skipped with a warning.

    Returns (mean_mae, [(stem, mae), ...]).  Output format identical to compute_regen_error.
    """
    ...
    regen_map = {
        ...
        for p in list_images(path_A_regen)
    }
    ...
    device = torch.device("cpu")  # no model needed; use CPU for tensor math
    for p in a_paths:
        stem = os.path.splitext(os.path.basename(p))[0]
        if stem not in regen_map:
            skipped += 1
            continue

        img_a = Image.open(p).convert("RGB")
        img_a_prime = Image.open(regen_map[stem]).convert("RGB")

        x = tfm(img_a).unsqueeze(0)
        x_prime = tfm(img_a_prime).unsqueeze(0)

        x_255 = (x + 1.0) * 127.5
        x_prime_255 = (x_prime.clamp(-1.0, 1.0) + 1.0) * 127.5
        err = torch.abs(x_255 - x_prime_255).mean(dim=1).squeeze(0)  # [H, W]

        stems.append(stem)
        error_maps.append(err.numpy())
        orig_tensors.append(x.squeeze(0))

    if skipped:
        print(f"[regen_error] Warning: {skipped} A tiles had no matching A' tile in {path_A_regen}")

    if not stems:
        raise RuntimeError(f"No matched pairs found between {path_A} and {path_A_regen}")
...
def main():
    ...
    ap.add_argument("--metric",
                    choices=["fid", "patch_ssim", "lpips", "regen_error"],
                    default="fid",
                    help="Metric to compute: fid (unpaired), patch_ssim/lpips (paired), "
                         "regen_error (cycle A→B'→A', requires --path_A --model --ckpt)")
    ap.add_argument("--path_real", default=None, type=str,
                    help="Folder with real target-domain images (required for fid/patch_ssim/lpips)")
    ap.add_argument("--path_fake", default=None, type=str,
                    help="Folder with generated images (required for fid/patch_ssim/lpips)")
    ...
    ap.add_argument("--path_A_regen", type=str, default=None,
                    help="Folder of pre-computed A' tiles (output of B→A inference). "
                         "When provided with regen_error, skips internal model inference and "
                         "computes MAE(A, A') directly — works for all models including cyclediffusion. "
                         "--model and --ckpt are not required in this mode.")
    ap.add_argument("--model", choices=["cyclegan", "unit", "munit", "dclgan", "uvcgan"], default=None,
                    help="Model type (required for regen_error without --path_A_regen)")
    ap.add_argument("--ckpt", type=str, default=None,
                    help="Model checkpoint path (required for regen_error without --path_A_regen)")
    ...
    # Validate path_real / path_fake for metrics that need them
    _needs_paired = args.metric in ("fid", "ssim", "patch_ssim", "lpips")
    if _needs_paired:
        for val, flag in [(args.path_real, "--path_real"), (args.path_fake, "--path_fake")]:
            if val is None:
                ap.error(f"--metric {args.metric} requires {flag}")

    # Build tissue filter once; None means disabled (all tiles included)
    tissue_stems = None
    if args.min_tissue_fraction > 0 or args.mask_dir:
        ref_path = args.path_real or args.path_A
        if ref_path is None:
    ...
    if args.metric == "fid":
        ...
        if args.save_csv:
            _save_csv(args.save_csv, ["metric", "value", "n_real", "n_fake"],
                      [{"metric": label, "value": f"{fid_like:.6f}",
        ...
        if args.path_A_regen is not None:
            # Precomputed-A' mode: no model inference needed (works for all models incl. cyclediffusion)
            if args.path_A is None:
                ap.error("regen_error with --path_A_regen requires --path_A")

            mean_mae, per_image = compute_regen_error_precomputed(
                path_A=args.path_A,
                path_A_regen=args.path_A_regen,
                image_size=args.ssim_image_size,
                overlay_dir=args.overlay_dir,
                save_error_npy=args.save_error_npy,
                tissue_stems=tissue_stems,
            )
            print(f"Regen Error MAE (precomputed A', path_A={args.path_A}): {mean_mae:.4f}")
            print(f"N_images={len(per_image)}, image_size={args.ssim_image_size}")
        else:
            # Internal-inference mode: runs A→B→A inside evaluation.py
            for flag, name in [(args.path_A, "--path_A"), (args.model, "--model"), (args.ckpt, "--ckpt")]:
                if flag is None:
                    ap.error(f"regen_error requires {name} (or use --path_A_regen for precomputed A' tiles)")
