#!/bin/bash
#SBATCH --job-name=i2i_ens_unit_b2a
#SBATCH --output=logs_ensemble/unit_infer_B2A_%A_%a.out
#SBATCH --error=logs_ensemble/unit_infer_B2A_%A_%a.err
...
ENSEMBLE_ROOT=/work2/bz66izin-VSproject/ensemble/unit/data_large/model_medium
CKPT="${ENSEMBLE_ROOT}/models/model_${MEMBER}/checkpoints/step_750000.pt"

# Input: A→B translated tiles produced by infer_ensemble_unit.sh
...
if [ ! -f "${CKPT}" ]; then
    ...
    echo "        Run train_ensemble_unit.sh first."
    exit 1
fi
...
if [ ! -d "${IN_DIR}" ] || [ -z "$(ls -A "${IN_DIR}" 2>/dev/null)" ]; then
    ...
    echo "        Run infer_ensemble_unit.sh first."
    exit 1
fi
