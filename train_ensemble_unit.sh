#!/bin/bash
#SBATCH --job-name=i2i_ens_unit_train
#SBATCH --output=logs_ensemble/unit_train_%A_%a.out
#SBATCH --error=logs_ensemble/unit_train_%A_%a.err
...
# medium UNIT: ngf=128, n_blocks=10, n_blocks_shared=2 (matches train_all_54.sh)
NGF=128
NBLOCKS=10
NSBLOCKS=2

OUTPUT_DIR=/work2/bz66izin-VSproject/ensemble/unit/data_large/model_medium/models/model_${MEMBER}
...
run_cmd python "${PROJECT_ROOT}/train.py" \
    ...
    --steps 750000 \
    --unit_ngf "${NGF}" \
    --unit_n_blocks "${NBLOCKS}" \
    --unit_n_blocks_shared "${NSBLOCKS}" \
    --dataA "${DATA_A}" \
    --dataB "${DATA_B}" \
