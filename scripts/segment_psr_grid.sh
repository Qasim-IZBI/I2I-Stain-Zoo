#!/bin/bash
#SBATCH --job-name=i2i_seg_grid
#SBATCH --output=logs_ensemble_grid/seg_grid_%A_%a.out
#SBATCH --error=logs_ensemble_grid/seg_grid_%A_%a.err
...
#
# Decomposition matches recon_ensemble_grid.sh exactly:
#   RANGE_ID  = TASK_ID / 50        (0-4  -> subset)
#   MEMBER_ID = (TASK_ID % 50) / 5  (0-9  -> member 01-10)
#   WSI_ID    = TASK_ID % 5         (0-4  -> folder 001-005)
#
# Input  : {subset}/model_small/reconstructed/model_{NN}/
# Output : {subset}/model_small/wsi_masks/model_{NN}/
#
...
if [ ! -f "${RECON_TIF}" ]; then
    ...
    echo "        Run recon_ensemble_grid.sh first (same array index)."
    exit 1
fi
