#!/bin/bash
#SBATCH --job-name=i2i_ens_dcl_b2a
#SBATCH --output=logs_ensemble/dclgan_infer_B2A_%A_%a.out
#SBATCH --error=logs_ensemble/dclgan_infer_B2A_%A_%a.err
...
# Input: A→B translated tiles produced by infer_ensemble_dclgan.sh
...
if [ ! -d "${IN_DIR}" ] || [ -z "$(ls -A "${IN_DIR}" 2>/dev/null)" ]; then
    ...
    echo "        Run infer_ensemble_dclgan.sh first."
    exit 1
fi
